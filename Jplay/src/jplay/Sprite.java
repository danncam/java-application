...
public class Sprite extends Animation {
    ...
    private double jumpVelocity = 15.9;//It's used for the jump,
    ...
    public void moveX(int leftKey, int rightKey, double velocity) {
        ...
        if (keyboard.keyDown(leftKey) && this.x > 1/* && onFloor*/) { // O booleano comentado é pra impedir que o sprite anime durante o pulo.
            this.x -= velocity;
            direction = LEFT;
            isMoving = true;
        }

        if (keyboard.keyDown(rightKey) && this.x + this.width < Window.getInstance().getWidth()/*&& onFloor*/) { // Same as above.
            this.x += velocity;
            direction = RIGHT;
            isMoving = true;
        }
    ...
    public void moveY(int upKey, int downKey, double velocity) {
        Keyboard keyboard = Window.getInstance().getKeyboard();
        if (keyboard.keyDown(upKey) && this.y > 30) { // 30 pela decoração da janela
            this.y -= velocity;
            direction = UP;
            isMoving = true;
        }
    ...
    public void autoMove() {
        if (this.y > 0 && this.y + this.height < Window.getInstance().getHeight() && this.x > 0 && this.x + this.width < Window.getInstance().getWidth()) {
            switch (direction) {
                case UP:
                    this.y -= this.height;
                    break;
